...
xgb_search = RandomizedSearchCV(
    ...
    xgb_params, n_iter=N_CONFIGS, scoring='r2', cv=3, verbose=0, random_state=42
)
xgb_search.fit(X_train_resid, y_train_resid)

rf_search = RandomizedSearchCV(
    RandomForestRegressor(random_state=42),
    rf_params, n_iter=N_CONFIGS, scoring='r2', cv=3, verbose=0, random_state=42
)
rf_search.fit(X_train_resid, y_train_resid)

gb_search = RandomizedSearchCV(
    GradientBoostingRegressor(random_state=42),
    gb_params, n_iter=N_CONFIGS, scoring='r2', cv=3, verbose=0, random_state=42
)
